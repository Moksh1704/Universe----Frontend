...
const EventCard = ({ event }) => {
  ...
  const handleRegister = async () => {
    // If backend provides a Google Form link, open it
    const formLink = event.google_form_url || event.form_url || event.registration_link || event.formLink;
    if (formLink) {
      try {
        const canOpen = await Linking.canOpenURL(formLink);
        if (canOpen) { await Linking.openURL(formLink); return; }
      } catch (_) {}
    }
    ...
    try {
      ...
      Alert.alert('Error', e.message || 'Could not register. Try again.');
    }
  };
  ...
  return (
    <View style={s.card}>
      ...
      <View style={s.cardBody}>
        ...
        <TouchableOpacity style={[s.regBtn, reg && s.regBtnDone]} onPress={handleRegister} activeOpacity={0.85}>
          <Ionicons name={reg ? 'checkmark-circle' : 'add-circle-outline'} size={16} color={reg ? COLORS.success : COLORS.secondary} />
          <Text style={[s.regBtnTxt, reg && s.regBtnDoneTxt]}>
            {(event.google_form_url || event.form_url || event.registration_link)
              ? 'Register via Form'
...
const s = StyleSheet.create({
  ...
  regBtnDoneTxt:{ color: COLORS.success },
});
